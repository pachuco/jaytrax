...
#include "resamplers/nearest.h"
#include "resamplers/linear.h"
#include "resamplers/cubic.h"
#include "resamplers/sinc.h"
...
void playerDestroy(Player *p)
{
    ...
    if (p) {
        ...
        if (p->resamp)         p->resamp->destroy(p->resamp);
...
Player * playerCreate(int SAMPLEFREQUENCY)
{
    ...
    for (i = 0; i < SE_MAXCHANS; i++)
        memset(&p->voices[i], 0, sizeof(Voice));
    
    //TODO: choose resamplers at runtime
    p->resamp = resamp_cubic_create();
                        ...
                        if ( p->channelNumber > 0 )
                        {
                            for (j = 0; j < p->channelNumber; ++j)
                            {
                                ...
                                if ( v->isSample == 1 )
                                {
                                    ...
                                    {
                                        //interpolation
                                        smp = p->resamp->interpSamp(p->resamp, v);
                                ...
                                {
                                    //interpolation
                                    smp = p->resamp->interpSynt(p->resamp, v);
